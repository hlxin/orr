...
class Chemisorption:
    ...
    def newns_anderson_semi(self, namodel_in, model, **kwargs):
        ...
        E_hyb_1 = energy_NA_1
        E_hyb_2 = 2*energy_NA_2
        E_hyb_3 = energy_NA_3

        E_rep_1 = 2*(na_1+f)*alpha*beta_1*vad2
        E_rep_2 = 2* 2*(na_2+f)*alpha*beta_2*vad2
        E_rep_3 = 2*(na_3+f)*alpha*beta_3*vad2

        
        E_hyb = energy_NA_1 + 2* energy_NA_2 + energy_NA_3
        E_rep = 2*(na_1+f)*alpha*beta_1*vad2 + 2* 2*(na_2+f)*alpha*beta_2*vad2 + 2*(na_3+f)*alpha*beta_3*vad2


        idx = torch.from_numpy(np.array(idx, dtype=np.float32)).to(device)
        ...
        parm = torch.cat((parm,wdos_1,wdos_2,wdos_3),dim=1)
        parm = torch.cat((parm,htwdos_1,htwdos_2,htwdos_3),dim=1)
